...
# Basic configuration
cluster="setonix"               # System-wide name of cluster, e.g. "setonix". This should match the name of the cluster, which can be checked
...
export GXBASE="${SOFTBASE}/${GXUSER}/SEA-pipe"          # Path to the base of the SEA Pipeline where the repository was 'git clone' into including the name of the repository foldername, e.g. "/software/projects/pawsey0272/kenil/SEA-pipe".
...
export GXCONTAINER="${SOFTBASE}/containers/gleamx_container_21Mar25.img"  # Absolute path to the GLEAM-X singularity container, including the file name, e.g. "$/software/projects/pawsey0272/containers/gleamx_container_21Mar25.img".
                                # This container is still being evaluated and available when requested from Kenil Ajudiya. In a future update
...
export GXSTANDARDQ="work"       # Slurm queue to submit tasks to, e.g. "workq". Available queues can be inspected using 'sinfo' on a system where the slurm schedular is available

# Compute node memory specification
export GXABSMEMORY=200          # Absolute memory a machine should be considered to have in GB, e.g. 60. This value is submitted to slurm via "--mem=${GXABSMEMORY}"
...
export GXMEMORY=190             # Typical memory a program should use in GB, e.g. 50. This is used for tasks like 'calibrate' and 'wsclean' to attempt to limit
...
export GXNCPUS=112              # Number of CPUs of each machine, e.g. 48. For tasks that have a 'core' like option this value is passed.
export GXNLCPUS=112             # Number of logical CPUs on each machine, e.g. 48. This option may be phased out.
export GXNPCPUS=64              # Number of physical CPUs on each machine, e.g. 24. This is meant to count only the physical cores available.
export GXNCPULINE="--nodes=1 --ntasks=1 --ntasks-per-node=1 --cpus-per-task=${GXNCPUS}"
                                # Informs the SLURM request how many CPUs should be allocated, e.g. "--ntasks-per-node=${GXNPCPUS}"
...
export GXSSH="${GXBASE}/ssh_keys/id_rsa"                            # Path to SSH private key to be used for archiving. If you direct it to a new generated key-pair. Keys can be generated with: ssh-keygen -t rsa -f "${GXBASE}/ssh_keys/gx_${GXUSER}"
if [[ -n "${GXSSH}" ]] && [[ ! -f "${GXSSH}" ]]; then               # Ensure restricted folder/file permissions, e.g. chmod -R 600 "${GXBASE}/ssh_keys".
...
export SINGULARITY_BINDPATH="${GXHOME}:${HOME},${GXSCRIPT},${GXBASE},${GXSCRATCH},${GXSSH},${GXMWALOOKUP}:/pb_lookup,${GXMWAPB},${GXSTAGE},/scratch/mwasci/asvo"
...
for var in GXCOMPUTER GXCLUSTER GXSTANDARDQ GXABSMEMORY GXMEMORY GXNCPUS GXNLCPUS GXNPCPUS GXUSER GXCOPYQ GXCOPYM GXLOG GXSCRIPT; do
    if [[ -z ${!var} ]]; then
